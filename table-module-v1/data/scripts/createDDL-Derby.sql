...
CREATE TABLE SALE (ID INTEGER GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY NOT NULL, DATE DATE, TOTAL DOUBLE, DISCOUNT_TOTAL DOUBLE, STATUS CHAR(1), CUSTOMER_ID INTEGER, CLOSED_AT DATE)
CREATE TABLE SALE_TRANSACTION (ID INTEGER GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY NOT NULL, SALE_ID INT NOT NULL, TRANSACTION_TYPE INT NOT NULL, TRANSACTION_VALUE DOUBLE NOT NULL, CREATED_AT DATE NOT NULL, FOREIGN KEY(SALE_ID) REFERENCES SALE(ID))
