package business;

import javax.persistence.PersistenceException;

import presentation.RegistTransactionService;

/**
 * Handles use case process sale (version with two operations: 
 * newSale followed by an arbitray number of addProductToSale) 
 * 
 * @author fmartins
 *
 */
public class ProcessSaleHandler {
	
	/**
	 * The sale's catalog
	 */
	private SaleCatalog saleCatalog;

	/**
	 * The customer's catalog
	 */
	private CustomerCatalog customerCatalog;
	
	/**
	 * The product's catalog
	 */
	private ProductCatalog productCatalog;
	
	private TransactionCatalog transactionCatalog;
	
	/**
	 * The current sale
	 */
	private Sale currentSale;

	/**
	 * Creates a handler for the process sale use case given 
	 * the sale, customer, and product catalogs.
	 * 
	 * @param saleCatalog A sale's catalog
	 * @param customerCatalog A customer's catalog
	 * @param productCatalog A product's catalog
	 */
	public ProcessSaleHandler(SaleCatalog saleCatalog, CustomerCatalog customerCatalog, 
			ProductCatalog productCatalog, TransactionCatalog transactionCatalog) {
		this.saleCatalog = saleCatalog;
		this.customerCatalog = customerCatalog;
		this.productCatalog = productCatalog;
		this.transactionCatalog = transactionCatalog;
	}

	/**
	 * Creates a new sale
	 * 
	 * @param vatNumber The customer's VAT number for the sale
	 * @throws ApplicationException In case the customer is not in the repository
	 */
	public Sale newSale (int vatNumber) throws ApplicationException {
		Customer customer = customerCatalog.getCustomer(vatNumber);
		currentSale = saleCatalog.newSale(customer);
		return currentSale;
	}

	public void closeSale(RegistTransactionService transactionService) throws ApplicationException{
		
		// checks if sale is already closed or not
		if(currentSale.getStatus() != SaleStatus.OPEN)
			throw new ApplicationException("This sale is already closed");
		
		// get sale customer to compute discount
		Discount discount = currentSale.getCustomer().getDiscount();
		double saleDiscount = discount.computeDiscount(currentSale);
		
		// compute discount value
		double saleTotal = 0;
		for(SaleProduct sp : currentSale.getSaleProducts())
			saleTotal += (sp.getQty() * sp.getProduct().getFaceValue());
		
		System.out.println("SALE TOTAL IS: " + saleTotal);
		
		// regist debit transaction the set close
		try{			
			transactionService.registDebitTransaction(currentSale.getId(), (saleTotal - saleDiscount));
		} catch (Exception e) {
			throw new ApplicationException("Error registering debit transaction", e);
		}
		
		// update sale values and persist
		currentSale.setDiscount(saleDiscount);
		currentSale.setTotal(saleTotal);
		currentSale.setStatus(SaleStatus.CLOSED);
		this.saleCatalog.updateSale(currentSale);
		
	}
	
	/**
	 * Adds a product to the current sale
	 * 
	 * @param prodCod The product code to be added to the sale 
	 * @param qty The quantity of the product sold
	 * @throws ApplicationException When the sale is closed, the product code
	 * is not part of the product's catalog, or when there is not enough stock
	 * to proceed with the sale
	 */
	public void addProductToSale (int prodCod, double qty) throws ApplicationException {
		Product product = productCatalog.getProduct(prodCod);			
		currentSale = saleCatalog.addProductToSale(currentSale, product, qty);
	}

	/**
	 * @return The sale's discount, according to the customer discount type
	 * @throws ApplicationException When some persistence error occurs
	 */
	public double getSaleDiscount () throws ApplicationException  {
		return currentSale.discount();
	}

	/**
	 * @return The sale's total, before discount.
	 * @throws ApplicationException When some persistence error occurs
	 */
	public double getSaleTotal() {
		return currentSale.total();
	}
	
	public Sale getCurrentSale(){
		return this.currentSale;
	}
	
	public void makePayment(RegistTransactionService transactionService){
		//first regist credit transaction then set payed
		try{
			
			// make transaction
			try{
				double value = currentSale.getTotal() - currentSale.getDiscount();
				transactionService.registCreditTransaction(currentSale.getId(), value);				
			} catch (Exception e) {
				throw new PersistenceException("Error registering debit transaction", e);
			}
			
			// set payed			
			currentSale.setStatus(SaleStatus.PAYED);
			saleCatalog.updateSale(currentSale);
			
		} catch (Exception e) {
			throw new PersistenceException("Error registering payment", e);
		}
	}
}
